...
public class MembreSQL {
  /**
   * Sélectionne une liste de membre dans la BD
   *
   * @param noMembre Le numéro de membre
   * @return Le résultat de la requête
   */
  static public ArrayList<HashMap> selectListeMembre(int noMembre) {
    DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
    Date date = new Date();
    date.setYear(date.getYear() - 1);
    String datestring = dateFormat.format(date);

    String query = "SELECT no, nom, prenom FROM membre "
            + "WHERE no LIKE '#" + Integer.toString(noMembre) + "#' "
            + "AND derniere_activite>'" + datestring + "'";
   ...
   *
   * @param noMembre Le numéro de membre
   * @param desactive Vrai pour obtenir comptes désactivés
   * @return Le résultat de la requête
   */
  static public ArrayList<HashMap> selectListeMembre(int noMembre, boolean desactive) {
    if (!desactive) {
      return selectListeMembre(noMembre);
    }
   ...
   * Sélectionne une liste de membre dans la BD
   *
   * @param nom Le prénom ou le nom de membre
   * @return Le résultat de la requête
   */
  static public ArrayList<HashMap> selectListeMembre(String nom) {
    DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
    Date date = new Date();
    date.setYear(date.getYear() - 1);
    String datestring = dateFormat.format(date);
    ...
    if (noms.length == 1) {
      like = " prenom LIKE '#" + noms[0] + "#' OR nom LIKE '#" + noms[0] + "#' ";
    } else {
      like = " prenom LIKE '#" + noms[0] + "#' AND nom LIKE '#" + noms[1] + "#'  ";
    }

    String query = "SELECT no, nom, prenom FROM membre WHERE" + like
            + "AND derniere_activite>'" + datestring + "'";
    return executeQuery(query);
  }
   ...
   * @return Le résultat de la requête
   */
  static public ArrayList<HashMap> selectListeMembre(String nom, boolean desactive) {
    if (!desactive) {
      return selectListeMembre(nom);
    }
